...
log "installing uv"
run_gui wine "${PYTHON_WIN_EXE}" -m pip install uv >/tmp/python-uv-install.log 2>&1 || {
  cat /tmp/python-uv-install.log >&2
  fail "uv installation failed"
}

log "verifying uv installation"
run_gui wine "${PYTHON_WIN_EXE}" -m pip show uv >/tmp/python-uv-show.log 2>&1 || {
  cat /tmp/python-uv-show.log >&2
  fail "uv installation verification failed"
}
cat /tmp/python-uv-show.log
